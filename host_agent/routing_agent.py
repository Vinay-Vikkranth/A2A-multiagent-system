...
from health_care_multiagent.host_agent.remote_agent_connection import (
...
class RoutingAgent:
    ...
    # initializes the RoutingAgent with remote agents details.
    def __init__(
        self,
        task_callback: TaskUpdateCallback | None = None,
    ):
        # function to notify the task update
        self.task_callback = task_callback
        # eg: {"WeatherAgent": http://localhost:10001}
        self.remote_agent_connections: dict[str, RemoteAgentConnections] = {}
        # eg: {"WeatherAgent": <AgentCard for WeatherAgent>}
        self.cards: dict[str, AgentCard] = {}
        # eg: {"name": "WeatherAgent", "description": "Provides weather info"}'
        self.agents: str = ''
    ...
    async def _async_init_components(
        ...
        # This creates one HTTP client to be reused for all network requests 
        async with httpx.AsyncClient(timeout=30) as client:
            # remote_agent_addresses is a list of URLs sent as parameters - eg: ["http://localhost:10001"]
            for address in remote_agent_addresses:
                # A2ACardResolver is used to get the agent card
                ...
                try:
                    ...
                    # Create Connection to the remote agent to send messages and receive responses
                    remote_connection = RemoteAgentConnections(
                        agent_card=card, agent_url=address
                    )
                    # eg: "WeatherAgent": <RemoteAgentConnections object>
                    self.remote_agent_connections[card.name] = remote_connection
                    # eg: "WeatherAgent": <AgentCard object>
    ...
    def create_agent(self) -> Agent:
        ...
        return Agent(
            ...
            description=(
                'This Routing agent orchestrates the decomposition of the user asking for weather forecast or airbnb accommodation or hellow world'
            ),
            tools=[
                self.send_message,
            ],
    ...
    def root_instruction(self, context: ReadonlyContext) -> str:
        ...
        return f"""
        **Role:** You are an expert Routing Delegator. Your primary function is to accurately delegate user inquiries regarding weather or accommodations or sleep or currency or diet plan to the appropriate specialized remote agents.
        ...
        * Currently Active Seller Agent: `{current_agent['active_agent']}`
                """
    # Check if there is an active agent in the context
    ...
    # Makes sure the session state is initialized before the agent starts working
    ...
    # Sends a task (message) to a specific remote agent.
    ...
    async def _async_main() -> Agent:
        routing_agent_instance = await RoutingAgent.create(
            remote_agent_addresses=[
                os.getenv('SLEEP_AGENT_URL', 'http://localhost:10011'),
                os.getenv('DIET_AGENT_URL', 'http://localhost:10012'),
            ]
        )
